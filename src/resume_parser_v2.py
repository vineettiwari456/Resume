...
class ResumeParser:
    def __init__(self):
        ...
        self.designations = []
        self.cities = []
        self.obj_company_skill = CompanyData()
    ...
    def seperation_block_index(self, main_keywordlist, raw_data):
                ...
                if len(word_indexs) > 0:
                    ...
                    for word_index in word_indexs:
                        ...
                        if kl + "." in str(raw_data).strip()[word_index:word_index + len(kl) + 10]:
                            ...
                            # if len(self.found_keword(self.designations,raw_data[word_index:][:150])):
                            dict_map[kl + "_" + str(b)] = word_index
                            b += 1
                            try:
                                main_keywordlist.remove(kl)
                            except:
        ...
        for m, n in raw_temp.items():
            if len(n) == 1 and list(n)[0] == "Experience":
                if s > 0:
                    n = {"Experiences"}
                    sort_keyword_index.update({"Experiences": m})
                s += 1
            # print(m, max(n))
            dict_map[max(n)] = m
        # print(dict_map)
        ...
        for m in op_dict.keys():
            ...
            if len(op_dict[m]) == 2:
                ...
                main_text = str(raw_data).strip()[x:].replace(":", '').strip()
            if main_text:
    ...
    def get_candidate_name(self, blockdict):
        pers_name = ""
        rdic = list(blockdict)
        rdc = ["Profile_Name", "profile_details", "skills", "work_experiences", "qualifications",
               "education_and_training",
               "extra_activity", "summary", "Academic_achievement"]
        # rdic.remove("Profile_Name")
        # rdic.insert(0,"Profile_Name")
        for j in rdc:
            rawname = self.obj_ner.get_candidate_name(blockdict.get(j, '').replace("(", " ").replace(")", " "))
            if rawname:
                pers_name = rawname
                break
        return pers_name
    ...
    def get_current_keyword_index(self, raw_workexp_text, companylist):
        ...
        for kl in companylist:
            ...
            word_indexs, offsets_id, key_index = self.get_string_match_index(str(raw_workexp_text).strip().lower(),
                                                                             kl.lower())
            # raw_workexp_text = raw_workexp_text.lower().replace(kl.lower(),' ')
            if len(word_indexs) > 0:
    ...
    def get_current_duration(self, year_duration, is_current=True, is_birth=False):
        ...
        if currentyear:
            ...
            rawyears = re.findall(
                regex,
                currentyear.replace("th", ' ').replace("nd", ' ').replace(",", ""))
            # print(rawyear)
            if len(rawyears) > 0:
                temp_date = []
                ...
                for yr in rawyears:
                    try:
                        dval = yr[0]
                        if "/" in dval:
                            if len(dval) > 5 and len(dval) < 7:
                                for i in dval.split("/"):
                                    if len(str(i)) == 4:
                                        dval = dval + "/20"
                                    break
                            dval1 = [j for j in dval.split("/") if len(j) > 1]
                            if len(dval1) < 1:
                                dval = ""
                        # print(dval)
    ...
    def get_multiple_same_index(self, ini_dict):
        ...
        for key, value in ini_dict.items():
            ...
            rev_dict.setdefault(value, set()).add(key.split("_")[0])
        # print(rev_dict)
        return rev_dict
    ...
    def get_individual_designation_work_exp(self, rawworkexp, st, end, st_check, is_startdate):
        ind_designation = ""
        designation_text = ""
        is_previous = False
        if st < 50:
        ...
        if st > 0 and end:
            if st_check:
                found_ind_desg = self.obj_company_skill.found_designations(rawworkexp[st - 50:st + 20])
                # found_ind_desg = self.found_keword(self.designations, rawworkexp[st - 45:st + 20])
                if len(found_ind_desg) > 0:
                    ind_designation = found_ind_desg[-1].strip()
                    is_previous = True

                ls_year_temp = self.get_years(rawworkexp[st - 32:st + 5])
                if len(ls_year_temp) > 0:
                    for d in ls_year_temp:
                        if "to" in d.lower():
                            ...
                            start_dat, end_dat, birday = self.get_current_duration(d,
                                                                                   is_current=False)
                            if start_dat:
                            # if not str(d).strip().startswith("95") and len(d)>3:
                                ls_year.append(d)
                            break
            else:
                # print(rawworkexp[st - 30:st])
                # found_ind_desg = self.obj_company_skill.found_designations(rawworkexp[st - 30:st])
                # if len(found_ind_desg) > 0:
                #     ind_designation = found_ind_desg[0].strip()
                #     is_previous = True
                if is_startdate:
                    ls_year_temp = self.get_years(rawworkexp[st - 30:st])
                else:
                    ls_year_temp = self.get_years(rawworkexp[st:st + 119])
                if len(ls_year_temp) > 0:
                    for d in ls_year_temp:
                        if "to" in d.lower():
                            ls_year.append(d)
            ...
            if not st_check:
                ...
                found_ind_desg = self.obj_company_skill.found_designations(rawworkexp[st:300])
            else:
                # found_ind_desg = self.found_keword(self.designations, rawworkexp[st - 40:st + 20])
                found_ind_desg = self.obj_company_skill.found_designations(rawworkexp[st - 40:st + 20])
            if len(found_ind_desg) > 0:
                ind_designation = found_ind_desg[0].strip()
                is_previous = True
        ld = 350
        if len(ls_year) == 0:
            ...
            for ls in ls_year_temp:
                if ls.lower().startswith("since") and "to" in ls.lower():
                    ...
                    if "/" in str(ls):
                        if len([i for i in str(ls).strip().split("/") if len(i)==2])!=2:
                            ls_year.append(ls)
                    else:
                        ls_year.append(ls)
                    break
        if ind_designation == "":
            ...
            found_ind_desg = self.obj_company_skill.found_designations(designation_text[:ld])
            if len(found_ind_desg) > 0:
                ind_designation = found_ind_desg[0].strip()
        if len(ls_year) == 0:
            if end:
        ...
        if ind_designation == "":
            ...
            found_ind_desg = self.obj_company_skill.found_designations(designation_text[:95])
            if len(found_ind_desg) > 0:
                is_previous = True
                ind_designation = found_ind_desg[0].strip()
    ...
    def get_complete_work_experince(self, cmpdic, rawworkexp, current_designation):
        ...
        if len(new_dict) == 1:
            for m, n in enumerate(new_dict):
                ...
                found_ind_de = self.obj_company_skill.found_designations(rawworkexp[:200])
                ind_designation = ""
                ind_designat = ""
            ...
            for mk, jk in enumerate(new_dict):
                ...
                if len(datatp) == 2:
                    start_check = True
                    # keyword_index = self.found_keword(self.designations, rawworkexp[datatp[0]:datatp[1]][:100])
                    keyword_index = self.obj_company_skill.found_designations(rawworkexp[datatp[0]:datatp[1]][:100])
                    if len(keyword_index) > 0:
                        if jk[1] < rawworkexp.index(keyword_index[0]) :
                            # yrcheck = self.get_years(rawworkexp[jk[1]:rawworkexp.index(keyword_index[0])])
                            # if len(yrcheck)==0 and self.get_years(rawworkexp[jk[1]-20:jk[1]]):
                            start_check = False
                    ...
                    keyword_index = self.obj_company_skill.found_designations(rawworkexp[datatp[0]:datatp[0]+100])
                    if len(keyword_index) > 0:
                        if jk[1] < rawworkexp.index(keyword_index[0]):
                            start_check = False
                    designation_text, is_previous, list_year = self.get_individual_designation_work_exp(rawworkexp,
                ...
                if len(temp_exp.get("desc", "")[len(temp_exp.get("company", "").get("name", "")):]) >= 15:
                    if temp_exp.get("start_date", "") != "" or temp_exp.get("end_date", "") != "":
    ...
    def get_temp_company_index(self, ini_dict, raw_text):
        ...
        for mf in max_value:
            if mf[0] > 35 and len(mf) >= 2:
                if (max([j - i for i, j in zip(mf[:-1], mf[1:])]) > 150 and min(
                        [j - i for i, j in zip(mf[:-1], mf[1:])])) >= 10:
                    h=0
                    for j in mf:
                        if h==0:
                            if j>=40:
                                yrls = self.get_years(raw_text[j - 40:j + 200])
                            else:
                                yrls = self.get_years(raw_text[j:j + 200])
                        else:
                            if j>=40:
                                yrls = self.get_years(raw_text[j-42:j + 200])
                        h+=1
                        if len(yrls) > 0:
                            # temp_id.append(mf[0])
                            temp_id.append(j)
            ...
            #     temp_id.extend(mf)
                ...
                if mf[0]>=50:
                    yrls = self.get_years(raw_text[mf[0] - 50:mf[0] + 300])
                    if len(yrls)>0:
                        temp_id.append(mf[0])
                else:
                    temp_id.append(mf[0])
    ...
    def extract_block(self, filepath, resume_text=None):
        # raw_data = self.getText(self.filepath)
        a = time.time()
        if resume_text:
            raw_dat = str(resume_text)
        else:
            raw_dat = self.get_docx_doc_pdf_text(filepath)
        if len(' '.join(raw_dat.split())) > 50:
            # print(raw_dat)
            ...
            raw_city_data = segmented_dic.get("Profile_Name", "") + " " + segmented_dic.get(
                                                 ...
                                                 "education_and_training", "")
            # print(raw_city_data)
            found_cities = self.obj_company_skill.found_cities(raw_city_data)
            # print('============================',found_cities)
            city = ""
            if len(found_cities) > 0:
                city = str(found_cities[0]).strip().capitalize()
            ...
            # raw_phone_data = segmented_dic.get("Profile_Name", "") + " " + segmented_dic.get(
            #     "profile_details", "") + " "
            ...
            elif 'birth date' in raw_data.lower():
                dobtext = raw_data.lower().split("birth date")[-1]
            if dobtext:
            ...
            resume_dict["personal_info"] = {
                "phones": [''.join(str(j).replace("(", "").replace(")", "").split()) for j in list(set(mobile_number))],
                "name": candidate_name,
                "emails": list(set(email_id)),
                "location": location_uuid, "date_of_birth": birthdate}
            print('eeeeeeeeeee', time.time() - e)
            hg = time.time()
            cmp_data_raw = segmented_dic.get("work_experiences", "").lower() + " " + segmented_dic.get("skills",
                                                                                                       "").lower()
            ...
            if " • • • • " in cmp_data_raw:
                pvt_ltd_raw_data = ' '.join(cmp_data_raw.split(" • • • • ")[:-1])
            else:
                pvt_ltd_raw_data = cmp_data_raw
            found_cmp = self.obj_company_skill.found_company(pvt_ltd_raw_data.lower())
            print(found_cmp)
            ...
            print('======>>', found_cmp_ind)
            d = time.time()
            found_designation = self.obj_company_skill.found_designations(raw_data.lower())
...
if __name__ == "__main__":
    ...
    # path = "doc_file/Pankaj Sunal_Java.pdf"
    # path ="doc_file/105_VikramReddy Gajjala.pdf"
    ...
    # path = "doc_file/Resume_Single_Column2.docx"
    ...
    # path = "doc_file/Vivek_IIIT_Allahabad_Salmon.pdf"
    # path = "doc_file/Rahul_kumar_Noida_4.08_yrs.docx"
    # path = "doc_file/Vinay_Sharma_Paytm.pdf"
    # path = "doc_file/Amit_15_Exp_Pune.docx"
    # path ="doc_file/vikas_Kalra_Noida_17.01_yrs.docx"
    # path ="doc_file/Akshit Bhandari_Product Manager_CRM.pdf"
    # path ="doc_file/Pallavi Gupta_Product Manager_Infoedge.pdf"
    # path ="doc_file/Akshit Bhatia_Java_Sopra.docx"
    # path ="doc_file/Tushar Gerg_Product Manager.pdf"
    # path ="doc_file/Sahil Sharma_9 yrs_Times Internet.docx"
    # path ="doc_file/Gaurank Sharma-Java.docx"
    # # path ="doc_file/Rishabh Jain_Product Manager_Career360.pdf"
    # path ="doc_file/Naivedya Jain_3+ yrs_Startup.pdf"
    # path = "doc_file/Navendu_Saxena_9 yrs_Product Management.pdf"
    # path = "doc_file/Navneet Hanjra_3.5 yrs_Elastic Search.docx"
    # path = "doc_file/Neeraj Nauni_1.6 yrs_Java.pdf"
    # path = "doc_file/Prashant Bhardwaj_Junglee Games.docx"
    # path ="doc_file/Raminder Kaur -Product Manager -4.5Years.pdf"
    # path = "doc_file/Arun Kumar Singh.pdf"
    # path = "doc_file/Aish Resu (1).docx"
    # path ="doc_file/90000251.pdf"
    # # path = "doc_file/90000724.pdf"
    # path = "doc_file/90000556.pdf"
    # path = "doc_file/96766724.pdf"
    # path = "doc_file/96528617.docx"
    # path = "doc_file/90408933.pdf"
