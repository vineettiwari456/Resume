    ...
    def get_individual_designation_work_exp(self, rawworkexp, st, end, st_check, is_startdate):
        ...
        if st > 0 and end:
            if st_check:
                ...
                ls_year_temp = self.get_years(rawworkexp[st - 30:st + 5])
                ...
                if is_startdate:
                    ...
                    ls_year_temp = self.get_years(rawworkexp[st:st + 105])
                if len(ls_year_temp) > 0:
                    for d in ls_year_temp:
                        if "to" in d.lower():
                            ls_year.append(d)
            ...
            for mk, jk in enumerate(new_dict):
                ...
                if len(datatp) == 2:
                    ...
                    if len(keyword_index) > 0:
                        if jk[1] < rawworkexp.index(keyword_index[0]):
                            start_check = False
                    # ls_year_temp = self.get_years(rawworkexp[datatp[0]-30:datatp[0]])
                    # if len(ls_year_temp)>0:
                    #     is_start_date = True
                    designation_text, is_previous, list_year = self.get_individual_designation_work_exp(rawworkexp,
                                                                                                        datatp[0],
                                                                                                        datatp[1],start_check,is_start_date)
                    ...
                    designation_text, is_previous, list_year = self.get_individual_designation_work_exp(rawworkexp,
                                                                                                        datatp[0],
                                                                                                        None,start_check,is_start_date)
    ...
    def get_temp_company_index(self, ini_dict, raw_text):
        ...
        for mf in max_value:
            if mf[0] > 35 and len(mf) >= 2 :
                if (max([j-i for i, j in zip(mf[:-1], mf[1:])])>150 and min([j-i for i, j in zip(mf[:-1], mf[1:])]))>10:
                    for j in mf:
        ...
        if len(' '.join(raw_dat.split()))>50:
            ...
            found_cities = self.found_keword(self.cities,
                                             segmented_dic.get("Profile_Name", "") + " " + segmented_dic.get(
                                                 "profile_details", "") + " " + segmented_dic.get("work_experiences",
                                                                                                  "")+" "+segmented_dic.get("education_and_training",""), is_lower=False)
            city = ""
            ...
            if birthdate == "":
                ...
                stddate, enddat, birthdate = self.get_current_duration(raw_bdate_text,
                                                                   is_birth=True, is_current=False)
            resume_dict = {}
            resume_dict["personal_info"] = {"phones": [''.join(str(j).replace("(","").replace(")","").split()) for j in list(set(mobile_number))], "name": candidate_name,
                                            "emails": list(set(email_id)),
                                            "location": location_uuid, "date_of_birth": birthdate}
            print('eeeeeeeeeee', time.time() - e)
            hg = time.time()
            cmp_data_raw = segmented_dic.get("work_experiences", "").lower() +" "+segmented_dic.get("skills", "").lower()
...
if __name__ == "__main__":
    ...
    obj = ResumeParser()
    obj.extract_block(path, resume_text=None)
